import {
  ...
  export class CallGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
    ...
    private activeUsers = new Map<string, { doctorId?: number; callerId?: number }>();
    ...
    handleConnection(socket: Socket) {
      ...
      const doctorId = socket.handshake.query.doctorId ? Number(socket.handshake.query.doctorId) : null;
    const callerId = socket.handshake.query.callerId ? Number(socket.handshake.query.callerId) : null;

    if (doctorId || callerId) {
      this.activeUsers.set(socket.id, { doctorId, callerId });
      console.log(`User connected: DoctorID: ${doctorId}, CallerID: ${callerId}`);
    ...
    handleAnswerCall(@MessageBody() data: any, @ConnectedSocket() socket: Socket) {
